...
BigInteger operator*(const BigInteger& a, const BigInteger& x)
{
    BigInteger result(0), result1(0), result2(0), result3(0);
    BigInteger a1, a2, x1, x2;
    if (a.number.size() > 1) && (x.number.size() > 1) {
        for (int i = 0; i<a.number.size()/2; i++) {
            a1.push_back(a.numbers[i]);
            a2.push_back(a.numbers[(a.number.size()/2) + 1])
        }
        for (int i = 0; i<x.number.size()/2; i++) {
            x1.push_back(x.numbers[i]);
            x2.push_back(x.numbers[(x.number.size()/2) + 1])
        }
    result1 = a1*x1;
    result3 = a2*x2;
    result2 = (a1+a2)*(x1+x2) - result1 - result3;
    result1 = result1.mult(MaxNumber + 1);
    result = result1.mult(MaxNumber + 1) + result2.mult(MaxNumber + 1) + result3;
    } else {
